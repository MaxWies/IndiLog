...
View::View(const ViewProto& view_proto)
    : id_(gsl::narrow_cast<uint16_t>(view_proto.view_id())),
      ...
      aggregator_replicas_(view_proto.aggregator_replicas()),
      ...
      aggregator_node_ids_(static_cast<size_t>(view_proto.aggregator_nodes_size())),
    ...
    for (size_t i = 0; i < aggregator_node_ids_.size(); i++) {
        aggregator_node_ids_[i] = gsl::narrow_cast<uint16_t>(
            view_proto.aggregator_nodes(static_cast<int>(i)));
    }
...
View::StorageShard::StorageShard(const View* view, uint32_t shard_id,
    ...
    : view_(view),
      ...
      aggregator_nodes_(aggregator_nodes),
